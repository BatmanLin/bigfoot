<template>
  <div>
    ...
    <el-table v-if="switchTableFlag" :data="allChannels">
      <el-table-column label="设备ID" prop="devID"></el-table-column>
      <el-table-column label="播放类型" prop="displayType"></el-table-column>
      <el-table-column label="所属设备" prop="devName"></el-table-column>
      <el-table-column label="所在位置" prop="areas">1111</el-table-column>
      <el-table-column label="操作">
        <template>
          <div>
    ...
    <el-dialog :visible.sync="dialogAddVisible">
      <el-form :model="addDevInfor" label-width="120px" label-position="right" :inline="true" :rules="rules">
        ...
        <el-form-item label="设备类型" prop="displayType">
          <el-select v-model="playType" placeholder="请选择" @change="changeDevInput">
            <el-option label="H5stream" value="H5steram"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="设备名" prop="devName">
        ...
        <el-form-item label="设备用户" prop="devAdmin">
          <el-input v-model="addDevInfor.devAdmin" :disabled="!commonInputFlag"></el-input>
        </el-form-item>
        <el-form-item label="设备密码" prop="devPassword">
          <el-input v-model="addDevInfor.devPassword" :disabled="!H5streamInputFlag"></el-input>
        </el-form-item>
        <el-form-item label="设备IP" prop="devIP">
  ...
  data() {
    var devIDRule = (rule, value, callback) => {
      ...
      setTimeout(async () => {
        ...
        } else if (value.toString().length < 3 || value.toString().length > 7) {
          callback(new Error(value.toString().length));
        } else {
  ...
  methods: {
    ...
    async getH5streamSessin() {
      const { data: session } = await this.$http.get(
        "http://localhost:7070/api/v1/Login?user=admin&password=827ccb0eea8a706c4c34a16891f84e7b"
      );
    ...
    getAllDevInfor() {
      this.$http.post("/api/user/selectDevInfor").then(response => {
        ...
        console.log(1111, this.allDevInfor);
      });
    },
      ...
      for (let index = 0; index < this.allDevInfor.length; index++) {
        ...
        if (element.displayType === "H5stream") {
          const {
            ...
            "http://localhost:7070/api/v1/GetDeviceSrc?token=" +
              element.devID +
    ...
    async addH5streamDevice(
      ...
      await this.$http.get(
        "http://localhost:7070/api/v1/AddDeviceHik?name=" +
          strName +
          "&token=" +
          strToken +
          "&user=" +
    ...
    async deleteH5streamDevice(strToken) {
      await this.$http.get(
        "http://localhost:7070/api/v1/DelDevice?token=" +
          strToken +
